...
void CWaterShader::SetupClipping(ref_params_t* pparams, bool negative)
{
	...
	vDist = m_pCurWater->origin - pparams->vieworg;
...
{
	if (m_vViewOrigin[0] > m_pCurWater->entity->curstate.mins[0] && m_vViewOrigin[1] > m_pCurWater->entity->curstate.mins[1] && m_vViewOrigin[2] > m_pCurWater->entity->curstate.mins[2] && m_vViewOrigin[0] < m_pCurWater->entity->curstate.maxs[0] && m_vViewOrigin[1] < m_pCurWater->entity->curstate.maxs[1] && m_vViewOrigin[2] < m_pCurWater->entity->curstate.maxs[2])
		return true;
...
void CWaterShader::SetupRefract(void)
{
	...
	else
	{
		...
		vMaxs = gBSPRenderer.m_pWorld->maxs;
		//VectorCopy(gBSPRenderer.m_pWorld->mins, vMins);
		vMins = gBSPRenderer.m_pWorld->mins;
...
void CWaterShader::SetupReflect(void)
{
	...
	Vector vMins, vMaxs;
	AngleVectors(m_pViewParams->viewangles, vForward, vRight, vUp);
	...
	m_pWaterParams.cl_viewangles = m_pWaterParams.viewangles;
	glCullFace(GL_FRONT);
	glColor4f(GL_ONE, GL_ONE, GL_ONE, GL_ONE);
