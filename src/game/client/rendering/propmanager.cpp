...
====================
*/
const char* CPropManager::ValueForKey(entity_t* ent, const char* key)
{
...
void CPropManager::ParseEntities(void)
{
	...
	const char* szSky = ValueForKey(&m_pBSPEntities[0], "skyname");
	...
	const char* szSpecial = ValueForKey(&m_pBSPEntities[0], "specialfog");
...
void CPropManager::LoadEntVars(void)
{
	for (int i = 0; i < m_iNumBSPEntities; i++)
	{
		const char* pValue = ValueForKey(&m_pBSPEntities[i], "classname");
